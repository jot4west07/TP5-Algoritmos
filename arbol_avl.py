...
class BinaryTree:
    ...
    # by_level_desordenado es el metodo original by_level que muestra los niveles desordenadamente
    def by_level_desordenado(self):
        pendientes = Queue()
        if self.root is not None:
    ...
    # by_level_ordenado es una forma ordenada del metodo by_level para mosrar nivel (0,1,2,3,4,5 etc...)
    def by_level_ordenado(self):
    ...
    # Realizamos una búsqueda en anchura (BFS) para recolectar los nodos por nivel
        while pendientes.size() > 0:
            node = pendientes.attention()

        # Si el nivel no existe en el diccionario, lo inicializamos con una lista vacía
            if node.height not in niveles:
                niveles[node.height] = []

        # Añadimos el nodo al nivel correspondiente
            niveles[node.height].append(node.value)

        # Añadimos los nodos hijos a la cola para seguir con la búsqueda
            if node.left is not None:
                pendientes.arrive(node.left)
            if node.right is not None:
                pendientes.arrive(node.right)

    # Ahora imprimimos los nodos por nivel, de menor a mayor
        for nivel in sorted(niveles.keys()):
            print(f"Nivel {nivel}: {niveles[nivel]}")
